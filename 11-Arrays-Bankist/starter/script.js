...
const displayMovments = function (movments, sort = false) {
  containerMovements.innerHTML = ''; // resetuje nasza tablice kontener
...
const calcDisplayPrintBalance = function (acc) {
  acc.balance = acc.movements.reduce((acc2, mov) => acc2 + mov, 0);
...
btnTransfer.addEventListener('click', function (e) {
  ...
  const reseverAcc = accounts.find(
    acc => acc.username === inputTransferTo.value
  );
  console.log(amount, reseverAcc);
  inputTransferAmount.value = inputTransferTo.value = '';

  if (
    amount > 0 &&
    currentAccount.balance >= amount &&
    reseverAcc?.username !== currentAccount.username &&
    reseverAcc
  ) {
    currentAccount.movements.push(-amount);
    reseverAcc.movements.push(amount);
...
const mocmentsUSD = movements.map(function(mov){
  return mov * eurToUsd;
//  return 23;//uzwroci tylko 23 zamiast wszystkiego co w movements !
});
console.log(movements);
console.log(mocmentsUSD);


// To samo co wyzej tylko inaczej
...
console.log(movements);
console.log(deposits); // beda tylko te ktore maja wartosc >0 :D lol
...
console.log(owners);
console.log(owners.sort()); // Methoda sort sortuje juz w sama zmienną a nie pokazuje jej
console.log(owners);

console.log(movements);
// console.log(movements.sort());    // sortuje alfapetycznie a nie numerycznie
// console.log(movements);
